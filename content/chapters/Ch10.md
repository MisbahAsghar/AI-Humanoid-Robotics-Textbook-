...
**Options**:
- **Cloud**: Google TTS, Azure TTS, Amazon Polly (natural voices, 100-300ms latency)
- **Local**: Piper, Coqui TTS, espeak (lower quality, 50-100ms latency, offline)
...
**Choose Cloud ASR when**: Need streaming (real-time transcription during speech, not after silence), ultra-low latency (<100ms), speaker diarization (who is speaking)
...
- **Acceptable for**: Manipulation tasks (pick-and-place where actions take 5-10 sec → latency <30% overhead), infrequent commands (1-2 commands/minute), voice control demos (user tolerates delay)
- **NOT acceptable for**: Reactive dialogue (conversational agent should respond <1 sec for natural conversation), emergency commands ("stop now!" requires immediate response), teleoperation (100-1000 Hz control needs <10ms latency), continuous command stream (multiple commands/second)
