...
- **Manipulation tasks**: Discrete actions (pick, place, push) at 0.1-1 Hz → 1-2s LLM latency acceptable (robot waits for plan, then executes 5-10 second primitive)
  - Example: Pick cup (5s) → wait for LLM (1s) → place cup (5s) = 11s total, latency is <10% overhead
- **Humanoid balance**: Reactive control at 100-1000 Hz (ankle torque every 1-10ms) → 1-2s latency catastrophic (robot falls in <500ms without correction)
